	...
	if( isset($_REQUEST['iv-submit-listing']) && isset($_REQUEST['payment_gateway']) && $_REQUEST['iv-submit-listing']=='register' && $_REQUEST['payment_gateway']=='paypal'){	
		...
		if($userdata['user_login']!='' and $userdata['user_email']!='' and $userdata['user_pass']!='' ){
			...
			if ( !$user_id and email_exists($userdata['user_email']) == false ) {							
				...
				$expire_interval = get_post_meta($package_id, 'jobboard_package_initial_expire_interval', true);						
				$initial_expire_type = get_post_meta($package_id, 'jobboard_package_initial_expire_type', true);
				...
				update_user_meta($user_id, 'jobboard_package_id',$package_id);				
		...
		$jobboard_package_recurring= get_post_meta( $package_id,'jobboard_package_recurring',true);
		$package_cost=$jobboard_package_cost;
		// Cheek here Coupon ************
		$trial_enable= get_post_meta( $package_id,'jobboard_package_enable_trial_period',true);
		if( $jobboard_package_recurring=='on'  ){
			$package_cost=get_post_meta($package_id, 'jobboard_package_recurring_cost_initial', true);			
		}
		if($package_cost >0){
			$currencyCode = (isset($paypal_api_currency)) ? $paypal_api_currency : 'USD';
			$sandbox = ($paypal_mode == 'live') ? '' : 'sandbox.';
			$paymentName = $package_name;
			$paymentDescription = $jobboard_package_cost.' '.$currencyCode.' for '.$package_name .' at '. get_bloginfo();
			$page_name_registration=get_option('epjbjobboard_registration' ); 
			...
			if($jobboard_package_recurring=='on'){
				$period= get_post_meta( $package_id,'jobboard_package_recurring_cycle_type',true);
				...
				$dis_amount=0;
				$package_amount=$jobboard_package_cost;
				$post_count = $wpdb->get_row($wpdb->prepare("SELECT * FROM $wpdb->posts WHERE post_title = '%s' and  post_type='jobboard_coupon'",$coupon_code ));	
				if(is_array($post_count)){
					$coupon_name = $post_count->post_title;							 
					$current_date=$today = date("m/d/Y");							 
					$start_date=get_post_meta($post_count->ID, 'jobboard_coupon_start_date', true);
					$end_date=get_post_meta($post_count->ID, 'jobboard_coupon_end_date', true);
					$coupon_used=get_post_meta($post_count->ID, 'jobboard_coupon_used', true);
					$coupon_limit=get_post_meta($post_count->ID, 'jobboard_coupon_limit', true);
					$dis_amount=get_post_meta($post_count->ID, 'jobboard_coupon_amount', true);							 
					$package_ids =get_post_meta($post_count->ID, 'jobboard_coupon_pac_id', true);
					...
					if($today_time >= $start_time && $today_time<=$expire_time && $coupon_used<=$coupon_limit && $pac_found == '1' ){	
						$coupon_type= get_post_meta($post_count->ID, 'jobboard_coupon_type', true);
						update_post_meta($post_count->ID, 'jobboard_coupon_used', $coupon_used+1);
				...
				// Coupon End *****************************
				$jobboard_package_cost = $jobboard_package_cost- $dis_amount;
				$orderParams = array(
				'PAYMENTREQUEST_0_AMT' => $jobboard_package_cost,
				'PAYMENTREQUEST_0_SHIPPINGAMT' => '0',
				'PAYMENTREQUEST_0_CURRENCYCODE' => $currencyCode,
				'PAYMENTREQUEST_0_ITEMAMT' => $jobboard_package_cost
				);
				$itemParams = array(
				'L_PAYMENTREQUEST_0_NAME0' => $paymentName,
				'L_PAYMENTREQUEST_0_DESC0' => $paymentDescription,
				'L_PAYMENTREQUEST_0_AMT0' =>  $jobboard_package_cost,
			...
			if(is_array($response) && $response['ACK'] == 'Success') {
				...
				$peried=get_post_meta( $package_id,'jobboard_package_recurring_cycle_type',true);
				$cycle_count=get_post_meta( $package_id,'jobboard_package_recurring_cycle_count',true);
				update_user_meta($userId, 'iv_paypal_recurring_profile_amt', get_post_meta( $package_id,'jobboard_package_recurring_cost_initial',true));
				update_user_meta($userId, 'jobboard_payment_gateway','paypal-express'); 
				update_user_meta($userId, 'iv_paypal_recurring_profile_init_amt',get_post_meta( $package_id,'jobboard_package_cost',true));
				...
				update_user_meta($userId, 'jobboard_discount',$dis_amount);
		...
		if($tokenRow){	
			...
			if( is_array($checkoutDetails) && ($checkoutDetails['ACK'] == 'Success') ) {
				...
				if($jobboard_package_recurring=='on'){
					...
					if( $trial_enable=='yes'  ){
						$initAmt='0';
						$period = get_post_meta( $package_id,'jobboard_package_recurring_trial_type',true);						
						$recurring_cycle_count=	get_post_meta( $package_id,'jobboard_package_trial_period_interval',true);	
					}	
					...
					$arb_limit	= get_post_meta( $package_id,'jobboard_package_recurring_cycle_limit',true); 
					if($arb_limit!=''){
						$recurringParams['TOTALBILLINGCYCLES']=$arb_limit;
					}
					$trial_enable= get_post_meta( $package_id,'jobboard_package_enable_trial_period',true); 
					if( $trial_enable=='yes' ){					
						$trial_amount= get_post_meta( $package_id,'jobboard_package_trial_amount',true); 
						$trial_period_interval= get_post_meta( $package_id,'jobboard_package_trial_period_interval',true); 
						$trial_type = get_post_meta( $package_id,'jobboard_package_recurring_trial_type',true);
					...
					if( is_array($recurringPayment) && $recurringPayment['ACK'] == 'Success' ) {
						...
						update_user_meta($userId, 'jobboard_exprie_date',$begins); 
						update_user_meta($userId, 'jobboard_package_id',$package_id);
						...
						$dis_amount=$discount;	
						require_once( wp_jobboard_ABSPATH.'inc/order-mail.php');
					...
					if( is_array($singlePayment) && $singlePayment['ACK'] == 'Success') {
						...
						update_user_meta($userId, 'jobboard_exprie_date',$exp_d); 
						update_user_meta($userId, 'jobboard_package_id',$package_id);
						// End expire date
	...
	if( isset($_REQUEST['iv-submit-upgrade']) &&  $_REQUEST['iv-submit-upgrade']=='upgrade' && $_REQUEST['payment_gateway']=='paypal' ){	
		...
		if($package_cost >0){
			...
			if(is_array($response) && $response['ACK'] == 'Success') {
				...
				update_user_meta($userId, 'iv_paypal_recurring_profile_period',$peried);	
				update_user_meta($userId, 'iv_paypal_recurring_cycle_count',$cycle_count);
				update_user_meta($userId, 'jobboard_discount','');
				update_user_meta($userId, 'iv_paypal_recurring_profile_desc',$recurringDescriptionFull);
			}	
			wp_redirect( 'https://www.'.$sandbox.'paypal.com/webscr?cmd=_express-checkout&token=' . urlencode($token) ); 
			exit; 
