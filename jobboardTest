		...
		final class wp_jobboard {
			...
			*/
			public function __construct() {
				...
				add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), array( $this, 'jobboard_plugin_action_links' ) );
					...
					// For elementor
				// Will add later  add_action( 'init', array($this, 'jobboard_elementor_file') );
				// Will add later  add_action( 'elementor/elements/categories_registered', array($this, 'add_elementor_widget_categories' ));
				// *** End elementor
				
				//---- COMMENT FILTERS ----//		
				add_action('init', array($this, 'remove_admin_bar') );	
				add_action( 'init', array($this, 'jobboard_paypal_form_submit') );
				add_action( 'init', array($this, 'jobboard_stripe_form_submit') );
				...
				add_action( 'wp_loaded', array(&$this, 'jobboard_woocommerce_form_submit') );
			...
			*/
			...
			public function include_template_function( $template_path ) { 
				$jobbank_directory_url=get_option('epjbjobboard_url');					
			...
			public function jobboard_check_field_input_access($field_key_pass, $field_value, $user_id,$template='myaccount'){ 
				...
				if(isset($field_type[$field_key_pass]) && $field_type[$field_key_pass]=='textarea'){	 
					...
					$return_value=$return_value.'<textarea  placeholder="'.esc_html__('Enter ','jobboard').esc_attr($field_value).'" name="'.esc_html($field_key_pass).'" id="'. esc_attr($field_key_pass).'"  class="form-textarea col-md-12"  rows="4"/>'.esc_attr(get_user_meta($current_user_id,$field_key_pass,true)).'</textarea></div></div>';
				}
			...
			public function jobboard_apply_submit_nonlogin(){
				...
				if ( 0 < $_FILES['file']['error'] ) {
					echo json_encode(array("code" => "success","msg"=>esc_html__( 'File Error:file missing', 'jobboard')));	
			...
			public function add_elementor_widget_categories() {
				...
				\Elementor\Plugin::$instance->elements_manager->add_category(
					'jobboard',
					[
						'title' => __( 'JobBoard', 'ivdirectories' ),
						'icon'  => 'fa fa-plug',
					]
				);

			}
			public function jobboard_elementor_file(  ) {
				if ( in_array( 'elementor/elementor.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
				
					include(wp_jobboard_template . 'elementor/elementor-single-meta.php');
					include(wp_jobboard_template . 'elementor/listing-maps.php');
					include(wp_jobboard_template . 'elementor/image-gallery.php');
					include(wp_jobboard_template . 'elementor/featured-image.php');
					include(wp_jobboard_template . 'elementor/apply-button.php');
			...
			public function jobboard_stripe_form_submit(  ) {
				require_once(wp_jobboard_DIR . '/admin/pages/payment-inc/stripe-submit.php');
			}
			...
			*/
			...
			public function jobboard_form_wizard_func($atts = '') {
				...
				if($current_user->ID==0){
					...
					if($signup_access=='0'){
						esc_html_e( 'Sorry! You are not allowed for signup.', 'jobboard' );
						}else{
			...
			public function check_listing_expire_date($listin_id, $owner_id,$directory_url){ 					
				$exp_date= get_user_meta($owner_id, 'jobboard_exprie_date', true);
				if($exp_date!=''){
					$package_id=get_user_meta($owner_id,'jobboard_package_id',true);
					...
					$have_package_feature= get_post_meta($package_id,'jobboard_package_feature',true);										
					if($have_package_feature=='yes'){
						if(strtotime($exp_date) < time()){
