<?php
	...
	wp_enqueue_style('bootstrap-jobboard-110', wp_jobboard_URLPATH . 'admin/files/css/iv-bootstrap-4.css');
...
<?php
	...
	if(get_post_meta($id, 'jobboard_featured', true)!='featured'){
		...
		// Remove all script tags and their contents from the excerpt
		$excerpt = preg_replace('/<script\b[^>]*>(.*?)<\/script>/is', '', $excerpt);
		$dir_data['descriptionshort']=$excerpt;
		$feature_img='';
	...
	wp_enqueue_script('bootstrap.min-4-script-24', wp_jobboard_URLPATH . 'admin/files/js/bootstrap.min-4.js');
