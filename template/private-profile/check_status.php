...
global $wpdb;
	...
	$userId=$current_user->ID;
		$payment_gateway=get_option('jobboard_payment_gateway');
	...
	if($payment_gateway=='woocommerce'){ 
		...
		$status_result=0;
		$wpayment_status=get_user_meta($userId, 'jobboard_payment_woo', true);
		$package_id=get_user_meta($userId, 'jobboard_package_id', true);
		if($wpayment_status=='woo_new'){
			 include(wp_jobboard_DIR . '/admin/pages/payment-inc/woo_new_order.php');
		}				
		if($wpayment_status=='woo_update'){ 
			 include(wp_jobboard_DIR . '/admin/pages/payment-inc/woo_update_package.php');			
		}
		if($wpayment_status=='success'){ 
				$exprie_date= strtotime (get_user_meta($userId, 'jobboard_exprie_date', true));
				$current_date=strtotime(date('Y-m-d'));
			if(  $current_date > $exprie_date){ 
				include(wp_jobboard_DIR . '/admin/pages/payment-inc/woo_daily_check.php');
			}
		}
	}
}
