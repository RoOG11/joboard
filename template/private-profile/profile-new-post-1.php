			...
			<div class="tab-content">
					...
					<?php					
						...
						if($package_id=="" OR $package_id=="0"){  						
							...
							$membership_pack = $wpdb->get_results($sql);
							$total_package=count($membership_pack);								
							if($total_package>0){		  echo'111111111'; 							
								$max=get_post_meta($package_id, 'jobboard_package_max_post_no', true);								
							}else{ 
								 $max=999999;
							}	
						...
						$sql=$wpdb->prepare("SELECT count(*) as total FROM $wpdb->posts WHERE post_type ='%s' and post_status IN ('publish','pending','draft') and post_author='%d'",$directory_url, $current_user->ID);									
						...
						?>
						<?php  esc_html_e('Please Upgrade Your Account','jobboard'); ?>
						<a href="<?php echo esc_url($reg_page).'?&profile=level'; ?>" title="Upgarde"><b><?php  esc_html_e('Here','jobboard'); ?> </b></a> 
						<?php  esc_html_e('To Add More Post.','jobboard'); ?>	
						<?php
							}else{
						?>					
						<div class="row">
